...
def get_convert_list_data(convert_type: str, convert_path: str | Path = CONVERT_JSON_PATH) -> Dict[str, Dict[str, str]]:
    logger.info(f"변환 리스트 데이터 가져오기: {convert_type} from {convert_path}")
    path = Path(convert_path)
    if not path.is_absolute():
        path = BASE_DIR / path
    list_data = read_json_file(path)
...
def load_lang_list(file_path: str = str(CONVERT_JSON_PATH)) -> list[list[str]]:
